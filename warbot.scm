...
(define (match-strings match)
  ;; Turn an irregex match object into a list of submatches
  (list-tabulate
   (+ 1 (irregex-match-num-submatches match))
   (lambda (idx) (irregex-match-substring match idx))))
  ...
  (regex-case
   ...
   (else
    (let ((matches 0))
      (for-each-privmsg-command nick
       (lambda (command)
         (let ((match (irregex-match (plugin-command-regexp command) body)))
           (if match
               (begin
                 (apply (plugin-command-handle! command) (append (list nick nick #f) (match-strings match)))
...
(define *mention-re*
  (irregex `(seq bos ,*botnick* ":" (* whitespace) (submatch (* any)) eos)))
...
(define (channel-mention nick channel body)
  ...
  (let* ((match (irregex-match *mention-re* body))
         (content (irregex-match-substring match 1)))
    (let ((matches 0))
      (for-each-channel-command nick
       (lambda (command)
         (let ((match (irregex-match (plugin-command-regexp command) content)))
           (if match
               (begin
                 (apply (plugin-command-handle! command) (append (list nick channel channel) (match-strings match)))
                 (set! matches (+ matches 1)))))))
